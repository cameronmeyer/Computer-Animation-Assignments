...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Rotation Angle a", rotateA)->setSpinnable(true);
	gui_1->addVariable("Rotation Angle b", rotateB)->setSpinnable(true);
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "main_object")
		{
			...
			// Create rotation matrix for local X-Axis rotation by totalAngleA degrees
			glm::mat4 rotateLocalX = glm::rotate(glm::mat4(1.0f), glm::radians(totalAngleA), glm::vec3(1.0f, 0.0f, 0.0f));
			main_object_model_mat = main_object_model_mat * rotateLocalX;

			// Create rotation matrix for world Y-Axis rotation by totalAngleB degrees
			glm::mat4 rotateWorldY = glm::rotate(glm::mat4(1.0f), glm::radians(totalAngleB), glm::vec3(0.0f, 1.0f, 0.0f));
			main_object_model_mat = rotateWorldY * main_object_model_mat;
